...
export default class WpClient {
  ...
  async getPosts(params: any = {}): Promise<{
    posts: Post[];
    totalPages: number;
  }> {
  ...
  async getPost(id: number): Promise<Post> {
    const response = await this.fetchWithHeaders(`${BASE_URL}posts/${id}`);

    return response.json();
  }

  async getMediaById(id: number): Promise<string> {
    const response = await this.fetchWithHeaders(`${BASE_URL}media/${id}`);

    return response.json();
  }

  async getCategoryById(id: number): Promise<string> {
    const response = await this.fetchWithHeaders(`${BASE_URL}categories/${id}`);

    return response.json();
  }

  async getAuthorById(id: number): Promise<string> {
    const response = await this.fetchWithHeaders(`${BASE_URL}users/${id}`);

    return response.json();
  }

  async getLatestThreePosts(): Promise<Post[]> {
    const response = await this.getPosts({ per_page: 3 });

    return response.posts;
  }

  async getCategoriesBySlug(slug: string): Promise<any> {
  ...
  async getCategories(): Promise<any> {
    const response = await this.fetchWithHeaders(`${BASE_URL}categories`);

    return response.json();
  }

  // async getAuthorBySlugAndPosts(slug: string): Promise<any> {
  //   const response = await this.fetchWithHeaders(
  //     `${BASE_URL}users?slug=${slug}`
  //   );
  //   const author = await response.json();
  //   const postsByAuthor = await this.getPosts({ author: author[0].id } as Partial<{ per_page: number; page: number; search: string; slug: string[]; author: number }>);
  ...
  private async fetchWithHeaders(url: string, method: string = "GET") {
